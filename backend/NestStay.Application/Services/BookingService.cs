...
public class BookingService : IBookingService
{
    ...
    // El contexto se necesita para manejar la transacción directamente
    ...
    public async Task<BookingResponse> CreateAsync(int guestId, CreateBookingRequest request)
    {
        ...
        // Iniciamos transacción Serializable para garantizar que no existan
        // dobles reservas bajo concurrencia
        using var transaction = await _context.Database
            .BeginTransactionAsync(IsolationLevel.Serializable);
        try
        {
            // Verificar disponibilidad DENTRO de la transacción
            // Esto bloquea las filas relevantes hasta que se haga commit
